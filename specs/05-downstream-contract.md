...
- **`engines` is now a method, not a property.** 3.x exposed `api.engines` as a cached property; 4.0 makes it `engines()` on both clients — `engines = api.engines()` (sync) / `engines = await api.engines()` (async) — because the async client can't expose a property that `await`s. Callers that read the attribute (e.g. `for e in api.engines`) must add the call (`for e in api.engines()`). `refresh_engine_cache()` is unchanged and still forces a refresh; `engines()` refreshes lazily on first use. **Breaking for downstream consumers** (the CLI and any code reading `api.engines`) — they must be updated in a paired change; tracked in [`99-open-questions.md`](./99-open-questions.md).
