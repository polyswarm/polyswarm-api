...
class PolySwarmAsyncAPI:
    ...
    async def engines(self):
        ...
        if not self._engines:
            await self.refresh_engine_cache()
        return self._engines

    async def refresh_engine_cache(self):
        """Refresh the cached engine listing."""
        engine_list = []
        async for engine in self._paginate(resources.Engine.list(self)):
    ...
    async def _finalize_sandbox_task(self, task):
    ...
    async def sandbox_file(
        ...
        return await self._finalize_sandbox_task(task)
