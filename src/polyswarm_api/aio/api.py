...
class PolySwarmAsyncAPI:
    ...
    async def engines(self):
        ...
        A method (not a property) so the async client can ``await`` the
        refresh; the sync mirror is generated by unasync like every other
        method. Call ``refresh_engine_cache()`` to force a refresh.
        """
