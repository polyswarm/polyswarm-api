...
class PolyswarmSession:
    ...
    def execute(self, request):
        ...
        """
        if request.result_parser is not None and not issubclass(
            request.result_parser, BaseJsonResource
        ):
            ...
            try:
                ...
                if created:
                    ...
                    try:
                        ...
                        logger.exception(
                            "Failed to close the partially-written download."
                        )
                    try:
                        os.remove(handle.name)
                    except Exception:
                        logger.exception(
                            "Failed to remove the partially-written download."
                        )
                raise
